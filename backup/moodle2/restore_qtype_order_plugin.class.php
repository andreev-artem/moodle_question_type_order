...
class restore_qtype_order_plugin extends restore_qtype_plugin {
    ...
    public function process_match($data) {
        ...
        if ($questioncreated) {
            ...
            // match questions require mapping of question_order_sub, because
            ...
            if ($sub) {
                ...
                throw new restore_step_exception('error_question_order_sub_missing_in_db', $data);
            }
        }
    }
        ...
        foreach ($rs as $rec) {
            ...
            foreach ($subquestionsarr as $key => $subquestion) {
                $subquestionsarr[$key] = $this->get_mappingid(
                        'question_order_sub', $subquestion);
            }
            $subquestions = implode(',', $subquestionsarr);
            $DB->set_field('question_order', 'subquestions', $subquestions,
                    array('id' => $rec->id));
        }
        $rs->close();
    }
    ...
    public function recode_response($questionid, $sequencenumber, array $response) {
        if (array_key_exists('_stemorder', $response)) {
            $response['_stemorder'] = $this->recode_order_sub_order($response['_stemorder']);
        }
        if (array_key_exists('_choiceorder', $response)) {
            $response['_choiceorder'] = $this->recode_order_sub_order($response['_choiceorder']);
        }
        return $response;
    }
